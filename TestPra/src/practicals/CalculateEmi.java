package practicals;
/**
 * Practical-8 A program to calculate EMI using Class, Object, Variables,
...
public class CalculateEmi {
	...
	public CalculateEmi(float amount, float percentage, float years) {
		...
		rate = rate / (12 * 100);
		time = time * 12;
		System.out.println("Your Per Month EMI Amount : "
				+ (principalAmount * rate * Math.pow(1 + rate, time) / (Math.pow(1 + rate, time) - 1)));
	}
